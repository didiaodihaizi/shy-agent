...
- [x] 3.1 启用 `+`：自绘菜单「添加技能」「添加文件」
- [x] 3.2 技能二级列表（可搜索、已启用）；文件走多选对话框
- [x] 3.3 Chip 行：技能/文件展示、移除、发送成功清空；样式对齐参考图
- [x] 3.4 图片 chip hover popover 预览
- [x] 3.5 `/` 选技能改为挂 skill chip（不再仅塞「使用技能」正文）
- [x] 3.6 `onSend` 把 skills/attachments 写入 `chat` 请求；读图中状态提示
